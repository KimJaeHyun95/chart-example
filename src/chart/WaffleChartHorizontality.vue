    ...
    createWaffleChart() {
      ...
      this.data.forEach((d, i) => {
        ...
        svg
            ...
            // 변경된 x 좌표 계산
            .attr('x', (d, j) => Math.floor((squareIndex + j) / rows) * (width / columns))
            // 변경된 y 좌표 계산
            .attr('y', (d, j) => ((squareIndex + j) % rows) * (height / rows))
